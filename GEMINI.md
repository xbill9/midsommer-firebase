...
6. **Firebase Integration & Leaderboard Bridge**: High scores are synchronized via `LeaderboardChannel` using standard JSON communication. If Firebase fails to initialize or is offline, the systems fallback to `SharedPreferences` cache (on mobile) or `localStorage` (on web) cleanly without breaking gameplay.
7. **Firebase Analytics & Logging**: Use the native `logEvent(name, params)` helper in `game.js` to dispatch custom gameplay metrics (e.g. `view_title_screen`, `level_start`, `level_playing`, `game_over`, `level_victory`). Avoid raw calls to native channel wrappers; utilize the helper method in `game.js`.
8. **Crashlytics Error Reporting**: JS-side exceptions are forwarded via `window.onerror` directly to Firebase Crashlytics on the native side. Any custom try-catch blocks in JS should log errors back using `LeaderboardChannel.postMessage(JSON.stringify({ type: 'recordError', message: e.message, stack: e.stack }))` when handling critical runtime operations.
9. **Firebase Performance Monitoring**: High-latency web operations (fetching leaderboard scores and posting score entries) are monitored on the Flutter side under performance traces named `get_leaderboard_scores` and `save_leaderboard_score`. Keep network payloads optimized to maintain fast latency metrics.
